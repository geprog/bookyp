<template>
  <div v-if="space">
    <div class="mt-4 flex flex-col p-4 border-primary-dark border-2 shadow-full text-center rounded-md">
      <h2 class="font-bold">{{ t('subscription.plan') }}</h2>

      <template v-if="space.plan === 'public'">
        <p>{{ t('subscription.public_plan_description') }}</p>

        <Button :text="t('subscription.make_space_free')" class="mx-auto mt-4" @click="changePlan('free')" />
      </template>
      <template v-else>
        <p>{{ t('subscription.free_plan_description', { amountOfMembers: spaceMembers.length }) }}</p>

        <div class="flex mt-4 justify-center gap-4">
          <a :href="mailtoUpgrade" class="w-1/2">
            <Button :text="t('subscription.upgrade')" class="w-full" />
          </a>

          <Button :text="t('subscription.make_space_public')" class="w-1/2" @click="changePlan('public')" />
        </div>
      </template>
    </div>
  </div>
